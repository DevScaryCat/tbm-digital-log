// app/api/org/profile/route.ts — 회사 공통 프로필(업종·공종) 전파 (감독자/단독 전용)
// 업종·공종은 개인 취향이 아니라 회사 속성 — 감독자 계정의 값이 원본이고, 저장 시 활성
// 현장 계정 전체에 전파한다. 성명·현장명·근로자 구분은 사람과 현장마다 다른 값이라 각자
// 자기 프로필에서 고치며, 여기서 덮어쓰지 않는다(덮으면 현장이 고친 값이 매번 되돌아간다).
// 본인 메타데이터는 클라이언트 updateUser가 이미 저장했으므로 여기서는 건드리지 않는다
// (이중 저장 레이스 방지) — 자식 전파만 담당.
        ...
        chunk.map(async (id) => {
          try {
            ...
            const { error } = await admin.auth.admin.updateUserById(id, {
              user_metadata: { ...m, industry, work_category: workCategory },
            });
            return !error;
          } catch {
            return false;
