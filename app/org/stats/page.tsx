 ...
 * states: loading · all(활동 기록+7일 차트+누적+위험요인+주의 현장) · site(현장 대시보드) · error(재시도)
 * tokens only — hairline depth, card radius 12px
 */

// 현장 통계 (감독자 전용) — 홈의 '통계 보기' 버튼으로 진입하는 전용 대시보드.
                ...
                {loading || ctxLoading ? (
                    ...
                    <>
                        <Select value={sel} onValueChange={setSel}>
                            <SelectTrigger className="w-full h-11 text-[14px] font-semibold border-cur-hairline rounded-[8px] bg-cur-card text-cur-ink">
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent className="bg-cur-card border-cur-hairline rounded-[12px]">
                                <SelectItem value="all">전체</SelectItem>
                                {memberSites.map((s) => (
                                    <SelectItem key={s.userId} value={s.userId}>{s.siteName}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                        ...
                        {selected ? (
                            ...
                            <>
                                ...
                                {/* 누적 — 전체 기간 합산 */}
                                <section className="bg-cur-card rounded-[12px] border border-cur-hairline p-5 space-y-3">
                                    <h2 className="text-[14px] font-bold text-cur-ink">누적 기록 <span className="text-[12px] text-cur-muted-soft font-medium">전체 기간 · 전 현장</span></h2>
                                    <div className="grid grid-cols-3 gap-px bg-cur-hairline border border-cur-hairline rounded-[12px] overflow-hidden text-center">
                                        <div className="bg-cur-card py-3.5">
                                            <p className="text-[11px] text-cur-muted font-semibold uppercase tracking-[0.6px] mb-1">TBM 회의록</p>
                                            <p className="text-[24px] leading-none font-bold text-cur-ink font-mono">{tMinutes}</p>
                                        </div>
                                        <div className="bg-cur-card py-3.5">
                                            <p className="text-[11px] text-cur-muted font-semibold uppercase tracking-[0.6px] mb-1">교육일지</p>
                                            <p className="text-[24px] leading-none font-bold text-cur-ink font-mono">{tLogs}</p>
                                        </div>
                                        <div className="bg-cur-card py-3.5">
                                            <p className="text-[11px] text-cur-muted font-semibold uppercase tracking-[0.6px] mb-1">근로자 제안</p>
                                            <p className="text-[24px] leading-none font-bold text-cur-ink font-mono">{tSuggestions}</p>
                                        </div>
                                    </div>
                                </section>
